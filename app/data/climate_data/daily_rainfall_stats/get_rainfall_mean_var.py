# Original data from CDS was of hourly large scale rain in GRIB format.
# Millimeters per hour (mm/hr).
# This script processes a GRIB file to extract daily mean and variance of large scale rain
# from specified locations and saves the results to CSV files.
# Daily mean and variance used to stochastically simulate daily large scale rain variations
# in the mammoth repopulation simulator.

# Locations: Krasnoyarsk (south taiga), Salekhard (north taiga), 
#            Saskylakh (south tundra), and Cape Chelyuskin (north tundra).

# Locations span a gradient of the siberian biomes from taiga (warmer) to tundra (cooler). 
# This is to get a better mapping of the UV variations across siberia.

import pygrib
import pandas as pd
from collections import defaultdict

locations = {
    "krasnoyarsk": (56.0, 93.0),
    "salekhard": (66.5, 66.5),
    "saskylakh": (71.9, 114.1),
    "cape_chelyuskin": (77.7, 104.3),
}

rain_data = {loc: defaultdict(list) for loc in locations}

grib_file_path = "C:/Users/aalad/Downloads/era5_rainfall_2024.grib" # file does not exist anymore due to space constraints

print("Starting GRIB file processing for large-scale rain.")

with pygrib.open(grib_file_path) as grbs:
    for i, grb in enumerate(grbs):
        if "large scale rain rate" not in grb.name.lower():
            continue

        date = grb.validDate
        data, lats, lons = grb.data()

        for loc, (target_lat, target_lon) in locations.items():
            lat_idx = (abs(lats[:, 0] - target_lat)).argmin()
            lon_idx = (abs(lons[0, :] - target_lon)).argmin()
            rain_mm_per_hr = data[lat_idx, lon_idx]
            rain_data[loc][date.date()].append(rain_mm_per_hr)

        if (i + 1) % 500 == 0:
            print(f"Processed {i + 1} messages...")

print("Computing daily mean and variance for large-scale rain now.")

for loc, day_map in rain_data.items():
    rows = []
    for day, rain_vals in sorted(day_map.items()):
        mean = sum(rain_vals) / len(rain_vals)
        var = sum((val - mean) ** 2 for val in rain_vals) / len(rain_vals)
        rows.append((day, mean, var))
        print(f"{loc}: {day} → Mean = {mean:.4f} mm/hr, Variance = {var:.4f}")

    df = pd.DataFrame(rows, columns=["Date", "Mean_Large_Scale_Rain_mmhr", "Variance_Large_Scale_Rain_mmhr"])
    df.to_csv(f"{loc}_daily_large_scale_rain_stats.csv", index=False)

print("\nFinished.")
